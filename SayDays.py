...
class SayDays:
    ...
    def is_leap(self) -> bool:
        ...
        # leap year ? then febrauary= 29: not= 28
    ...
    def days(self)-> int:
        ...
        total = 0
        m= 0 # oth month = January
        
        # calculate all days in all previous months
        while m < self.month -1: #0<4
            total += days_in_month[m]
            m += 1
        #add the days in this month until today( given date)
        total += self.day # property not funtion return
        return total


    def days_left(self):
        #until dec 31, how many days left? 365// 366
        total_days = 366 if self.is_leap() else 365
        return total_days - self.days() #not 16th,but

    def weekday(self):
        #Use zeller's formula to find the numeric daay of the week
        y = self.year
        m = self.month
        d = self.day
